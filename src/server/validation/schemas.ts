...
export const checkoutSchema = z.object({
  ...
  /** UPI reference/UTR the customer types after paying to the shop's QR. */
  paymentReference: trimmed(40).optional(),
...
export const adminProductSchema = z.object({
  ...
  paymentMode: z.enum(["both", "cod", "online"]).default("both"),
  deliveryDays: z.preprocess(
    (v) => (v === "" || v === null || v === undefined ? null : Number(v)),
    z.number().int().min(0).max(120).nullable().optional(),
  ),
  rating: z.preprocess((v) => (v === "" || v === null || v === undefined ? null : Number(v)), z.number().min(0).max(5).nullable().optional()),
  ratingCount: z.coerce.number().int().min(0).max(99999).default(0),
...
export const settingsSchema = z.object({
  ...
  onlineMode: z.enum(["qr", "gateway", "off"]).default("qr"),
  upiId: trimmed(80).optional(),
  upiPayeeName: trimmed(80).optional(),
  upiQrImage: trimmed(400).optional(),
  paymentInstructions: z.string().max(1200).optional(),
  utrRequired: z.coerce.boolean().default(false),
  notifyMobile: trimmed(20).optional(),
  notifyEmail: trimmed(160).optional(),
  notifyWebhookUrl: trimmed(400).optional(),
  notifyOrderEnabled: z.coerce.boolean().default(true),
